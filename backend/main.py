...
async def get_user_stories(user_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """Get stories for a specific user"""
    # Get stories that haven't expired for the specific user
    now = datetime.utcnow()
    ...
    return [
        StoryResponse(
            ...
            author={
                ...
                "first_name": story.author.first_name,
                "last_name": story.author.last_name,
                "avatar": getattr(story.author, 'avatar', None)
            },
            content=story.content,
            media_type=story.media_type,
            media_url=story.media_url,
        ...
        if tagged_user:
            ...
            if tagged_user.story_notifications:
                notification = Notification(
                    ...
                    title=f"{current_user.first_name} {current_user.last_name}",
                    message="marcou voc�� em um story",
